...
import { TrendingUp, Users, Activity, FileText, RefreshCw, Info } from 'lucide-react';
...
const AnalyticsPanel: React.FC = () => {
  ...
  // Generate usage data based on real stats
  const usageData = [
    { month: 'Jan', namaste: stats?.total_codes || 0, icd11: Math.floor((stats?.total_codes || 0) * 0.8), mappings: Math.floor((stats?.total_codes || 0) * 0.7) },
    { month: 'Feb', namaste: Math.floor((stats?.total_codes || 0) * 1.1), icd11: Math.floor((stats?.total_codes || 0) * 0.9), mappings: Math.floor((stats?.total_codes || 0) * 0.8) },
    { month: 'Mar', namaste: Math.floor((stats?.total_codes || 0) * 1.2), icd11: Math.floor((stats?.total_codes || 0) * 1.0), mappings: Math.floor((stats?.total_codes || 0) * 0.9) },
    { month: 'Apr', namaste: Math.floor((stats?.total_codes || 0) * 1.3), icd11: Math.floor((stats?.total_codes || 0) * 1.1), mappings: Math.floor((stats?.total_codes || 0) * 1.0) },
    { month: 'May', namaste: Math.floor((stats?.total_codes || 0) * 1.4), icd11: Math.floor((stats?.total_codes || 0) * 1.2), mappings: Math.floor((stats?.total_codes || 0) * 1.1) },
    { month: 'Jun', namaste: stats?.total_codes || 0, icd11: Math.floor((stats?.total_codes || 0) * 1.3), mappings: Math.floor((stats?.total_codes || 0) * 1.2) }
  ];

  const systemData = [
    { name: 'Ayurveda', value: stats?.systems?.ayurveda || 0, color: '#3B82F6' },
    { name: 'Siddha', value: stats?.systems?.siddha || 0, color: '#7C3AED' },
    { name: 'Unani', value: stats?.systems?.unani || 0, color: '#14B8A6' }
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {[
            { 
              ...
              value: stats?.total_codes?.toLocaleString() || '0', 
              change: '+12%', 
              icon: FileText, 
            ...
            { 
              ...
              value: `${Object.keys(stats?.systems || {}).length} Systems`, 
              change: '+8%', 
              icon: Users, 
